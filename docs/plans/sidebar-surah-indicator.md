...
### 4. Pinned surah for explicit taps (multi-surah pages)

The page-derived `activeSurah` (`surahs.findLast(startPage <= pageNumber)`) is correct for swipe/arrow arrival — the last surah to have started is what a reader considers current — but wrong for an explicit sidebar tap onto a page hosting more than one surah: it shows whichever surah on that page has the highest id, not the one tapped (e.g. tapping 112 or 113 both land on page 604 but the trigger shows 114). It never self-corrects until the reader leaves that page.

Fix: `SurahListItem`'s tap pins the picked id via `pinnedSurahId`/`setPinnedSurahId` on `SidebarContext`. `Sidebar` prefers the pinned surah over the page-derived guess **only while `pageNumber` is still within the pinned surah's `[startPage, endPage]`** (parsed from `pinnedSurah.pages`); the moment the reader leaves that range by any means the pin invalidates and page-derived resolution resumes. Swipe/arrow behaviour is untouched — the pin is set only from `SurahListItem`'s tap.

**Effect-keying gotcha (found live):** the pin-clearing effect must be keyed on `pageNumber` alone, not `[pinnedSurahId, pinnedSurahValid]`. `SurahListItem` sets the pin in the same handler that calls `window.history.replaceState`, but Next syncs `usePathname()` on a *later* render — an effect keyed on the pin re-runs while `pinnedSurahId` is set but `pageNumber` is still the pre-jump value, reads `pinnedSurahValid` false against the stale page, and clears the pin before `pageNumber` catches up, silently reproducing the bug. Keying on `pageNumber` alone defers the validity check to the render where the page actually changed. See the inline comment in `Sidebar.tsx`.
...
**`activeSurah` resolution:**

| Condition | `activeSurah` resolves to |
|---|---|
| `pinnedSurahId` set AND `pageNumber` ∈ that surah's `[startPage, endPage]` | the pinned surah |
| `pinnedSurahId` set but `pageNumber` outside its range | page-derived (`findLast`), and the pin clears |
| `pinnedSurahId` null | page-derived (`surahs.findLast(startPage <= pageNumber)`) |
...
- `app/contexts/SidebarContext.tsx` — add `SurahSlim`, `currentSurah`, `setCurrentSurah`; add `pinnedSurahId: number | null` + `setPinnedSurahId` to context value and provider state.
- `app/components/nav/Sidebar.tsx` — derive active surah from pathname+surahs; call `setCurrentSurah`; lift tabs to controlled state; add scroll refs and `useEffect` for auto-scroll. Compute `pinnedSurah` from `pinnedSurahId` + `surahs`, validate against `[startPage, endPage]` parsed from `pinnedSurah.pages`, prefer it over `findLast` when valid; clear the pin once it no longer matches `pageNumber` — effect keyed on `pageNumber` alone (see gotcha above).
...
- `app/components/SurahListItem.tsx` — add `data-surah-id={surah.id}` to the `Link`; in the same `onClick` branch that calls `jumpTo(surahStartingPage)`, also call `setPinnedSurahId(surah.id)` (from `useSidebar()`).
...
- **Multi-surah pages (swipe/arrow arrival):** surah is whichever starts on or before current page with the highest start-page — i.e. the last surah to have started. This matches what the user considers "current surah."
- **Multi-surah pages (explicit tap):** the tapped id is pinned and wins over the page-derived guess while the page stays in the pinned surah's range — otherwise tapping 112 or 113 onto page 604 would both show 114.
- **Stale pin re-encountered later:** the pin clears the instant `pageNumber` leaves its range, so it can never apply to an unrelated later visit to the same page.
- **Full-page navigation (no `jumpTo` — modifier-click, or pager not mounted):** `SurahListItem` does a real Next navigation and never pins — a fresh `Sidebar` mount could show the wrong surah on first paint for a multi-surah target. Rare, out of scope.
- **RubList's analogous `currentRub = rubs.findLast(...)`:** same class of bug is possible if two rubs' start-verses land on the same page. Not reported, rubs are evenly spaced, left as a noted latent risk.
...
- **After pinning 112, swipe backward to page 603:** page 603 ∉ [604,604] → pin clears, falls through to `findLast` → shows "Al-Masad" (111). Matches pre-existing swipe behavior.
- **After pinning a surah, swipe forward within its own multi-page range:** pin stays valid the whole time — no flicker to a different surah mid-surah.
- **Tap a single-surah-page surah (e.g. 2, Al-Baqarah, page 2):** pin=2, page 2 ∈ [2,49] → shows "Al-Baqarah" — same result page-derived logic already gave; pin is a no-op (not a regression).
...
- Pin only from `SurahListItem`'s explicit tap — never from swipe, arrows, keyboard, or `ContinueReadingLink` (none carry an unambiguous selected surah).
...
- Do not "fix" the multi-surah case by changing `findLast` to `find` — that only flips which swipe-arrival case is wrong without fixing tap-selection.
- Do not thread a surah id through `ReaderNavigationContext`'s `jumpTo(page: number)` signature — it's shared by `ContinueReadingLink` (page-only) and would need every caller updated for no benefit; `SidebarContext` already reaches both `SurahListItem` and `Sidebar`.
- Do not extend the pin fix to `RubList`/rubs in this task — not reported, latent risk only.
...
- Multi-surah disambiguation: an explicit tap pins its surah id (`SidebarContext.pinnedSurahId`), valid only while `pageNumber` stays in that surah's page range; swipe/arrow resolution is left untouched (#320).

## Revision History

- 2026-08-16 — folded Addendum (#320): wrong surah name on shared multi-surah pages — `pinnedSurahId` on `SidebarContext` lets an explicit tap override the page-derived `findLast` guess until the reader leaves the surah's page range.
