...
| Index deleted from Cache Storage offline | Error state + Retry (React Query retries ~7s before it lands); recovery restores the stashed bytes under the exact precache key and Retries — all offline, zero API calls throughout |
