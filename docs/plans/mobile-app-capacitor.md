...
- If signing in → system browser session (`ASWebAuthenticationSession` / Chrome Custom Tabs) → Universal/App Link callback → one-time backend session-bootstrap page sets the existing HTTPS web cookie inside the WebView (never pass tokens into JS).
...
- Auth/bootstrap gate: fresh install → system browser → link → session cookie present in WebView (`SameSite=None; Secure` verified readable by `getToken`); Google + Apple private-relay linking; no token in JS/`localStorage`.
...
- Root shell scaffold (done, slice 2): `capacitor.config.ts` (hosted `server.url` per environment via `CAP_SERVER_URL`, default production host, `allowNavigation` pinned, `androidScheme: https`; `appId: "app.furqan"` is pre-release and frozen at first store upload), `android/` + `ios/` native projects (standard CLI layout at repo root — inert for the web build), `@capacitor/{core,cli,android,ios}` 8.5.2 in `package.json`.
...
- `middleware.ts` (extend matcher exclusions): `/.well-known/apple-app-site-association` + `/.well-known/assetlinks.json` (else `intl-middleware` locale-prefixes them and Universal/App Links + AASA verification break).
...
- `app/sw.ts` (extend rule list, same pattern as marks/QDC tafsir rules): explicit `NetworkOnly` ahead of `...defaultCache` for the bootstrap GET, device-token status GETs, and any plans reads if awrad goes offline-capable. POST-only routes need no rule — document why per route.
