...
string, c: chapter_id }`. `useSearch` uses the API online and this index (same normalization, same
`take`/`skip` paging) when `navigator.onLine === false` or the fetch rejects. Surah search offline
filters the already-precached `chapters.json`. See [ADR 0062](../adr/0062-precached-search-index-offline-engine.md).
