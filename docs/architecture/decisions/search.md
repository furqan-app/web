...
**Decision:** Verse search works offline from a committed precached index,
`public/quran/search-index.json` (generated by `scripts/quran-search-index/generate.js`), holding
one row per verse: `{ k: verse_key, t: text_imlaei_simple, d: pre-joined qpc_uthmani_hafs display
string, c: chapter_id }`. `useSearch` uses the API online and this index when
`navigator.onLine === false` or the fetch rejects, with the same `take`/`skip` paging and the same
normalization as the matching online route: verse matching applies `normalizeArabicQuery`
(hamza-alif fold) only — no digit fold, `text_imlaei_simple` has no digits — while offline surah
search filters the already-precached `chapters.json` with the `normalizeDigits` numeric-id fold, the
same as `/api/search/chapters`. See [ADR 0062](../adr/0062-precached-search-index-offline-engine.md).

**Constraints:**
- The display string is the `Word.qpc_uthmani_hafs` join in `position` order — never
  `text_imlaei_simple` (simplified alif/hamza) and never `text_uthmani` / `words.text_uthmani`
  (renders waqf codepoints as plain circles under `UthmanicHafs1Ver18`; proven by a 2026-09-06
  browser experiment, reverted). The match column stays `text_imlaei_simple`.
- The index carries no page number and no surah names: page resolves at use time through the
  active mushaf edition's `verse-pages/{mushafId}.json`, names through `chapters.json` (ADR 0033).
  Word text is edition-agnostic; only placement differs per edition.
