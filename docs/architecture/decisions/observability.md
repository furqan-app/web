...
- The daily wird reminder supports up to `MAX_GENERAL_WIRD_REMINDERS` (3) general reminder rows per user, dedupe-keyed `plans.daily_reminder:<userId>:slot:<1-3>`, plus one dedicated reminder per active plan the user chooses to bind, dedupe-keyed `plans.daily_reminder:<userId>:plan:<planId>` (D1, revised — superseded the original single-row-per-user model). A plan bound to a dedicated reminder must be excluded from every general reminder's aggregate; `resolveGeneralWirdDispatch` and `resolveDedicatedWirdDispatch` enforce this split. Never create a reminder row with a dedupe key outside these two shapes. Each row still advances daily via `nextOccurrence`.
- A migration that rewrites `ScheduledNotification.dedupe_key` values must guard against a row already existing at the target key (e.g. a `NOT EXISTS`/`LEFT JOIN` check) — application code may already have created the new-shaped row on a given database before the migration runs there.
...
- Daily wird reminder dispatches use the `push` channel only (D7, revised) — the in-app notification surface (bell, feed UI, `in_app` channel and its store methods/routes) was removed entirely as a deliberate product decision; a user without push permission granted receives no reminder, with no in-app fallback. `email` must never be targeted. Removing a channel from the registry requires grepping for every caller of its store methods and UI, not just deleting the registry entry.
...
- **Weekly reminder recurrence (2026-09-13, #628, ADR 0070):** `ScheduledNotification.recurrence` accepts `"weekly"` alongside `"daily"`; `nextOccurrence`'s weekly branch advances by seven sequential DST-corrected single-day steps (reusing `advanceOneDay`, never a new date-math primitive) rather than tracking the target weekday itself — advancing by exactly 7 calendar days preserves it indefinitely. `app/api/cron/reminders/route.ts`'s three reschedule branches (skip-guard, resolver-failure, dispatch-success) must treat `"weekly"` exactly like `"daily"` (reschedule, never `completeReminder`/`failReminder`) — before this decision only `"daily"` rescheduled and every other recurrence value was terminally completed/failed on its first fire, which would have silently killed every weekly reminder after one send. `ScheduledNotification.weekday Int?` (0–6) is a new first-class column — the same class of scheduling metadata as the existing `timezone`/`locale` columns, not `payload` JSON, which stays reserved for notification *content* (D2). A dedicated (per-plan) reminder's `weekday` is derived from the plan's `CustomWirdDefinition.cadence.weekday` and mirrored onto the row at upsert time, never independently user-edited; a general reminder slot has no parent plan, so its `weekday` column is the row's own user-set value.
