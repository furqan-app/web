...
**`Word.audio_url`'s raw QDC file number must not be trusted as-is — the seeder rewrites it to always equal `Word.position`.** QDC fuses Rub-el-hizb marks (۞) and waqf/pause marks (ۖ ۗ ۘ ۙ ۚ ۛ) directly into the `text_uthmani` of the adjacent word (e.g. `"۞ فَخَلَفَ"`) rather than giving them their own `words[]` entry — `char_type_name` only ever takes `"word"` or `"end"`, never a mark type. But QDC's *internal audio-track numbering* still counts those fused marks as separate tracks, so in any verse with a preceding/embedded mark, the raw `audio_url` file number silently drifts ahead of `Word.position` (verified 2:26, 2:44, 19:59 — e.g. 19:59 word 1, the rub-mark-prefixed word, ships with `audio_url` pointing at file `002` instead of `001`). Since `Word.position` is already the correct sequential real-word ordinal per verse (verse-relative, 1-indexed, `"end"` always last), the fix is to force the trailing zero-padded number in `audio_url` to `String(position).padStart(3, "0")` for every `char_type_name === "word"` row with a non-null `audio_url`, discarding whatever number QDC returned. Validated against the full seeded DB (6,236 verses / 47,572 word-audio rows): this matches a full mark-detection-based derivation on all but one row, where it's the *safer* answer (a no-op on an inconsistent QDC gap). Do not attempt to detect/parse embedded mark characters to compute an offset — `position` alone is sufficient and provably correct for this dataset. See `docs/plans/word-audio-playback.md` Addendum (2026-07-15).
