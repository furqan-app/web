...
- **Chrome loses information as the viewport narrows; the mushaf never loses reading size** (ADR 0038). Below the desk band the rail relays into a bottom transport bar — reserving height, never width, so line length is untouched — and its physical-right placement is re-decided per band rather than inherited.
...
- **App chrome is an opaque surface, never glass** (subtask 3.2). `bg-background/75 backdrop-blur-md` over a `(7,15,23)` desk reads as a hole with the mushaf showing through, which is the entire reason dark needed `:root.theme-dark nav … { color: white !important }` on every descendant plus a counter-override to give the search dropdown its tokens back. `.fq-chrome-bar` supplies an opaque face, a top rim catch-light and a cast from one theme-agnostic rule; all three overrides are deleted, and dark's nav text measures 15.39:1 on semantic tokens alone.
- **The brand mark is identity, so it is a medallion and not `--primary`.** `.fq-icon-chip` filled the logo with the state accent. `.fq-medallion` is a struck seal — gold rim, dark face — in **every** theme, which is also what keeps the white logo silhouette readable (design-language.md §12). The account avatar is identity too, and lost its `--accent` fill for the same reason; its unread dot keeps `--primary` and is the only accent on that control.
- **Three device classes, and production now matches them** (subtask 5.1c). The compact boundary is `1023`, not `767`: production's old `768–1023` inset band was a fourth screen nobody had designed, and the user rejected it on sight. `MOBILE_QUERY` in `use-is-mobile.ts` and every compact block in `globals.css` must stay numerically identical. **Known consequence:** reading size in that band moves 26px → 28px, because it now comes from compact's font-from-width cap instead of the desktop card clamp. Upward, and within ADR 0038's presets.
