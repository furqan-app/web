...
- **There are four reading contracts, not three.** `<768` full-bleed single page; **`768–1023` single page with a real desk margin**; `1024–1366` full-bleed spread; `≥1367×800` the desk composition. A rule keyed on "below the desk band" is wrong for 768–1023 — that band has a surround and was built to have one.
- **Atmosphere keys off whether a desk exists, not off a breakpoint.** Lamp, vignette and nav arrows are dropped exactly where the page is full-bleed and kept where there is surround; they need something to act on. The page's own pool is unaffected everywhere, because it acts on the paper.
