...
- **PAUSED, not merged/shipped (2026-07-27):** non-Hafs riwaya audio (Warsh, Qaloon, Shoba, Qunbul, Al-Bazzi, Al-Douri, Al-Soosi) was built on `feature/143-riwaya-recitation-audio` as a second, structurally different provider (QuranHub) with a parallel `ended`-event-driven playback path (`playRiwaya`), since QuranHub has no shared-timeline audio file to attach `VerseTiming`s to. Shelved after confirming riwaya verse numbering can genuinely diverge from Hafs mid-surah (real qira'at counting differences, not a bug) — since mushaf display is always Hafs text, this let audio play content that didn't match the highlighted verse for affected surahs. Blocked on Trello #146 (word-level timestamps via self-alignment); once real per-riwaya `ChapterAudio`/`VerseTiming` data exists, riwaya should plug into the primary `RecitationProvider` engine instead of this second one. See ADR 0021 Addendum (2026-07-27) and `docs/plans/recitation-playback.md` Addendum 8's "Paused 2026-07-27" note for the full account. **This entry does not describe code on `main`** — it documents the paused branch's state for whoever resumes it.
