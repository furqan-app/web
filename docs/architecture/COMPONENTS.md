...
(page — server component; self reader is statically generated, grant reader is dynamic)
  ReaderPage                 — SHARED server body for both routes; takes basePath + optional grantId; builds THREE carousel panels (prev / current / next spreads) via buildPanel, each fetching its 2 pages — all six getPageWords calls SEQUENTIAL (ADR 0013) — for the tablet swipe carousel (ADR 0027); neighbor panels carry fq-carousel-side (CSS-hidden off tablet double-view); delegates @font-face injection to FontFaceInjector (client — ADR 0020) for all 6 page IDs; keeps <link rel="preload"> for the current page only; computes single-step (±1) and pair-step (±2) nav hrefs (per-panel via computeSpreadNav) + plain page-order prevPageNum/nextPageNum + prev/nextPairPageNum for swipe; derives the displayed page's first verse_key (getFirstVerseKeyOfPage) for RecitationPlayButton (desktop) and RecitationPageSync (mobile context bridge); renders FontFaceInjector + RecitationPageSync + toolbar (hidden md:flex: QuranSafhaViewToggle + RecitationPlayButton) + QuranSwipeNav (given the 3 panels) + QuranSpread
    FontFaceInjector           — "use client" leaf: injects @font-face rules for pair page IDs via <style dangerouslySetInnerHTML>; must be a client component to avoid RSC resource-hoisting hydration mismatches (ADR 0020). Reads QuranTajweedContext itself (ReaderPage is a Server Component and can't) — only when tajweedMode is true, additionally injects the COLRv1 tajweed @font-face + @font-palette-values (light/dark/gold) blocks for the pair (ADR 0023); --Light and --Gold palettes include override-colors remapping brand colors per CPAL slot (Addendum 13); --Dark palette retains base palette 1 without overrides pending dark-theme color review
    RecitationPageSync         — "use client" leaf: syncs the current page's firstVerseKey into RecitationContext.pageFirstVerseKey via useEffect; returns null; bridges the server-computed verse key to the mobile Nav's RecitationPlayButton, which cannot receive props from ReaderPage
    QuranSafhaViewToggle      — client pill (lg+ only): single/double icon buttons, reads/writes QuranSafhaViewContext
    RecitationPlayButton      — client: play/pause entry point; on desktop (inside ReaderPage toolbar) receives firstVerseKey as prop; on mobile (inside Nav, md:hidden) reads pageFirstVerseKey from RecitationContext — idle → starts playback at the page's first verse; active session → toggles play/pause via RecitationContext (ADR 0021)
    QuranSwipeNav            — "use client" wrapper: dual-path swipe animation + navigation; strip = 3 panels [next][current][prev]. MOBILE/DESKTOP (single-panel): neighbors are display:none, strip rests at translateX(0), drag tracks the finger 1:1, current flies off to translateX(±100%) on commit over 220ms (SINGLE_EXIT_MS) then router.push — the original single-slot behavior, unchanged. TABLET double-view (carousel = view==="double" && useIsTablet, matching the CSS scope — ADR 0027): CSS reveals neighbors + rests strip at translateX(-100%); drag anchors to that base and is amplified 1.5× (CAROUSEL_DRAG_GAIN, visual only — commit threshold stays on raw deltaX); commit slides the real neighbor to center (0% next / -200% prev) over 380ms (CAROUSEL_EXIT_MS) THEN router.push (no post-nav pop). Commits ≥ 80px, EASE_OUT; snaps back to the base offset (then clears inline transform) below threshold; a commit in flight blocks a second swipe (isCommitting) until the route lands; prefers-reduced-motion → instant router.push. Steps a PAIR in double view (view==="double" && useIsLgUp → pairStep), one page otherwise; hrefs are plain page-order (not locale-flipped). onClick toggles NavOverlayContext
...
NavOverlayContext            — tablet-only (md–lg-1) nav overlay state on /pages/ routes: overlayVisible, toggleOverlay (tap-to-toggle + 3s auto-hide), isOverlayMode; consumed by Nav (fixed+slide) and QuranSwipeNav (onClick trigger)
