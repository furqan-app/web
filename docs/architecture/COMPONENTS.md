...
(page — server component; self reader is statically generated, grant reader is dynamic)
  ReaderPage                 — thin async SSR entry (ADR 0028) shared by both routes (self = statically generated, grant = dynamic); takes basePath + optional grantId/viewingOwnerName. Fetches ONLY the current pair's words (getPageWords ×2, SEQUENTIAL per ADR 0013) and hands off to <ReaderPager>. The old 3-panel (was 5) carousel + buildPanel is gone; every subsequent page change is client-side in the pager (no router.push, no per-swipe remount)
    ...
    FontFaceInjector           — "use client" leaf: registers base @font-face fonts for the current WINDOW's page IDs (WOFF2 — /fonts/v1/woff2/pN.woff2) via the immutable page-font-registry (app/utils/page-font-registry.ts — ensurePageFonts; ADR 0029), never via a mutated <style> (a live stylesheet mutation resets every already-loaded FontFace in it to `unloaded`, blanking visible text — the root cause of the swipe-commit flicker). Keeps its own rolling LRU of the last MAX_KEPT=24 page IDs so swiping back never re-downloads a just-dropped font (ADR 0028), independent of the registry's own internal LRU. Client component to avoid RSC resource-hoisting hydration mismatches (ADR 0020). Reads QuranTajweedContext itself — only when tajweedMode is true, renders one immutable keyed `<style key={id}>` per page id (COLRv1 tajweed @font-face at /fonts/v4/colrv1/woff2/pN.woff2 + @font-palette-values light/dark/gold blocks, ADR 0023) — React mounts/unmounts whole elements on LRU change rather than rewriting a shared sheet. --Light and --Gold palettes remap brand colors per CPAL slot (Addendum 13); --Dark retains base palette 1 without overrides pending dark-theme color review
    page-font-registry.ts     — client-only util (app/utils/page-font-registry.ts): the immutable FontFace registry behind ADR 0029. `ensurePageFonts(ids)` creates `new FontFace('quran-p{id}', ...) → document.fonts.add → face.load()` for each unregistered id and never modifies a face after creation; a shared module-level LRU (cap 24) evicts via `document.fonts.delete(face)` on the single oldest face only. Used by both ReaderPager (mobile, single-id set) and FontFaceInjector (desktop/tablet, pair-expanded set) so all base-font registration funnels through one registry instance.
    RecitationPageSync         — "use client" null leaf: syncs the current page's firstVerseKey into RecitationContext.pageFirstVerseKey via useEffect; bridges the verse key to the mobile Nav's RecitationPlayButton, which cannot receive props from the pager
    ...
    RecitationPlayButton      — client play/pause entry point (rendered in Nav, not the reader entry): receives firstVerseKey as a prop where available, else falls back to pageFirstVerseKey from RecitationContext (mobile) — idle → starts playback at the page's first verse; active session → toggles play/pause via RecitationContext (ADR 0021)
