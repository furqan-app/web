...
(page — server component; self reader is statically generated, grant reader is dynamic)
  ReaderPage                 — thin async SSR entry (ADR 0028) shared by both routes (self = statically generated, grant = dynamic); takes basePath + optional grantId/viewingOwnerName. Fetches ONLY the current pair's words (getPageWords ×2, SEQUENTIAL per ADR 0013) and hands off to <ReaderPager>. The old 3-panel (was 5) carousel + buildPanel is gone; every subsequent page change is client-side in the pager (no router.push, no per-swipe remount)
    ...
    FontFaceInjector           — "use client" leaf: injects @font-face rules for the current WINDOW's page IDs (WOFF2 — /fonts/v1/woff2/pN.woff2) via <style dangerouslySetInnerHTML>; keeps a rolling LRU of the last MAX_KEPT=24 page IDs so swiping back never re-downloads a just-dropped font (ADR 0028). Client component to avoid RSC resource-hoisting hydration mismatches (ADR 0020). Reads QuranTajweedContext itself — only when tajweedMode is true, additionally injects the COLRv1 tajweed @font-face (/fonts/v4/colrv1/woff2/pN.woff2) + @font-palette-values (light/dark/gold) blocks (ADR 0023); --Light and --Gold palettes remap brand colors per CPAL slot (Addendum 13); --Dark retains base palette 1 without overrides pending dark-theme color review
