...
(page — server component; self reader is statically generated, grant reader is dynamic)
  ReaderPage                 — thin async SSR entry (ADR 0028) shared by both routes (self = statically generated, grant = dynamic); takes basePath + optional grantId/viewingOwnerName. Fetches ONLY the current pair's words (getPageWords ×2, SEQUENTIAL per ADR 0013) and hands off to <ReaderPager>. The old 3-panel (was 5) carousel + buildPanel is gone; every subsequent page change is client-side in the pager (no router.push, no per-swipe remount)
    ...
    RecitationPageSync         — "use client" null leaf: syncs the current page's firstVerseKey into RecitationContext.pageFirstVerseKey via useEffect; bridges the verse key to RecitationPlayerBar's play button (the bar cannot receive props from the pager)
...
```
components/ui/               — shadcn/ui primitives (button, command, dialog, dropdown-menu, input, popover, radio-group, sheet, switch, tabs, textarea)
...
RecitationSettingsSheet      — client: mounted app-wide alongside RecitationPlayerBar; controlled Sheet (RecitationContext's isSettingsOpen), not a SheetTrigger — opened only from the player bar's gear icon (MarkModal's "Play from here" plays directly and no longer opens this sheet, Addendum 6 of recitation-playback.md); a "Narration" pill row (Hafs + 7 riwayat, Addendum 8) sits above the reciter picker, resetting `reciterId` to null on change; reciter picker is a searchable Popover+Command combobox (ReciterCombobox, local helper) fed the one `reciters` list from context — same component for Hafs and every other riwaya, since both share the unified `Reciter` type — portals into the Sheet's own DOM node (captured via callback ref) rather than document.body, required to avoid Radix's Dialog/Popover focus-trap conflict, see DECISIONS.md; stop point (2-col icon-pill grid, options filtered to page/surah only when `riwaya !== "hafs"` — juz/hizb/rub/none need cross-chapter chaining, Hafs-only; "none" hides the whole-range repeat stepper, ADR 0021 Addendum 2026-07-16), repeat steppers, playback speed, and pause-between-repeats render unconditionally now — both engines support them (Addendum 8); each section icon-labeled via a shared SectionHeader helper; edits live/persisted settings, no separate "Play" CTA
```
...
RecitationContext            — recitation playback (ADR 0021, cross-chapter chaining added in Addendum 2026-07-16; second riwaya engine added Addendum 8/2026-07-27): owns the single <audio> element (mounted once, above the reader route tree so it survives page auto-advance and background playback), settings (stopPoint/repeat counts/speed/pause + `riwaya` + one unified `reciterId: string | null`, persisted to localStorage — QDC's numeric reciter ids are stringified at the provider boundary so Hafs and every other riwaya share one id field/one `reciters: Reciter[]` list instead of a parallel id+list per provider), and two independent playback engines dispatched from one `play(verseKey)`/one `ended` listener based on `settings.riwaya`: (1) the Hafs engine — QDC-backed, timeupdate handler drives word-level highlighting (direct DOM ref registry, not React state — timeupdate fires ~4x/sec) and page auto-advance (router.push when the recited verse's page falls outside the currently-visible page set), `ended` auto-loads the next chapter's audio when the resolved stopPoint (page/rub/hizb/juz/none, all of which may span a later chapter) hasn't been reached yet, or stops at 114:6/manual stop otherwise; (2) the riwaya engine (`playRiwaya`/`handleRiwayaVerseEnded`) — QuranHub-backed, no shared timeline exists so it plays one independent per-verse audio file at a time, advancing on the `<audio>` element's native `ended` event; whole-verse highlighting (`applyVerseHighlight`, toggles every wordRefRegistry entry prefixed `${verseKey}:`) instead of word-level; supports speed, per-ayah repeat, and range repeat (`scheduleRiwayaReplay`/`seekToRiwayaRangeStart`, file-swap-based mirrors of the Hafs engine's timestamp-based `scheduleSeek`/`seekToRangeStart`) and stop-points limited to `"page"`/`"surah"` (`resolveRiwayaStopVerseKey`, synchronous — `verses[]` already carries `.page` per entry, no DB call); `"juz"`/`"hizb"`/`"rub"`/`"none"` remain Hafs-only (would need cross-chapter chaining, out of scope). Mid-session: a reciter change (riwaya unchanged) reloads in place via one effect that branches internally on `settings.riwaya` to pick the QDC or QuranHub reload sequence; a stop-point change similarly branches (DB call for Hafs, synchronous for riwaya); a riwaya change itself stops playback (crosses engines — highlight mode and data source both change). Proxies QDC via app/utils/recitation-api.ts (app/api/quran/recitations/reciters, app/api/quran/recitations/[reciterId]/chapters/[chapterId], app/api/quran/chapters/[chapterId]/verse-pages), proxies QuranHub via the same file's fetchRiwayaReciters/fetchRiwayaChapterAudio (app/api/quran/recitations/riwaya/reciters, app/api/quran/recitations/riwaya/[editionIdentifier]/chapters/[chapterId]), and hits the DB-only app/api/quran/verses/[verseKey]/stop-point (no QDC, Hafs-only) to resolve page/rub/hizb/juz stop targets — see docs/architecture/adr/0021-recitation-playback.md
