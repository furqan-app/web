...
terminal. "Unanswered" means no terminal answer within 5 minutes (attended) — then the question
is escalated through `fq-ask-human`. Routine or unattended runs go to `fq-ask-human` directly
with an explicitly chosen timeout, raised for unattended runs. Presence test: `process.stdin.isTTY && process.env.CI !== 'true'` — agents infer attended vs unattended from their runtime context.
Use `onTimeout: "halt"` for scope and implementer-choice questions; `"default"` for low-stakes
picks only (the orchestrator decides *when* to escalate; the script decides *what happens* on timeout).
...
| **Review** | [`/review-fq-work`](review-work.md) (working tree) and/or `fq-delegate --lane second-opinion` | 1–N | Point every reviewer at the **ADRs and specs**, not only the plan's summary of them (`review-work.md` §3). For a **UI-affecting** task the orchestrator runs a browser smoke **first** — 1. production build via `npm run e2e:serve` (e2e DBs — never dev DBs, never `next dev`); 2. both locales × three themes on one static mushaf page + one stateful reader route; 3. persisted round-trip = theme + locale + reading position survive reload; 4. PWA-gated bits via [`pwa-testing.md`](../standards/pwa-testing.md) spoof snippets — because a static review plus unit tests structurally cannot catch layout / RTL / i18n / data-shape breakage. Fold findings via a delta brief to the implementer, reviewed like the first pass. |
