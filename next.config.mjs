...
const withSerwist = withSerwistInit({
  ...
  // `quran/reciters-{ar,en}.json` (ADR 0049) replace a live per-mount QDC call
  // from RecitationProvider with a precached static file, same treatment as
  // chapters.json above — generated by scripts/quran-reciters/generate.js.
  // `quran/search-index.json` (ADR 0062) is the offline verse-search index — same
  // static, immutable, rarely-changing shape as chapters.json above, generated by
  // scripts/quran-search-index/generate.js. ~2.2 MB raw / ~0.5 MB gzip: the one
  // deliberate addition to this list, accepted so offline search works with zero
  // prior online search (lazy-load was considered and rejected for failing the
  // first-ever-search-offline case).
  // `quran/verse-pages/*.json` (ADR 0033, #593) are the per-edition verse→page
  // maps — same static, immutable shape, generated by scripts/quran-json/generate.js.
  // ~72 KB per edition, globbed so future editions are covered. The maps were
  // previously runtime-cached only, so a prime fetch beating SW registration left
  // cold-offline loads with no map (CI on #593); precache makes them unconditional
  // for search and every other offline consumer (rub nav, edition switch).
  globPublicPatterns: [
    'icon.svg',
    'icons/**/*',
    'quran/chapters.json',
    'quran/search-index.json',
    'quran/verse-pages/*.json',
    'quran/reciters-ar.json',
    'quran/reciters-en.json',
    'launch.html',
    'offline-ar.html',
    'offline-en.html',
  ],
