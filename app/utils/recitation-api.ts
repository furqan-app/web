...
// Static committed file per locale (ADR 0049), not an API route — fetched
// directly, no jsonResponse envelope to unwrap. Generated by
// scripts/quran-reciters/generate.js; regenerate + redeploy when QDC's
// reciter roster changes.
export const fetchReciters = async (language: string): Promise<Reciter[]> => {
  const res = await fetch(`/quran/reciters-${language}.json`);
  return res.json();
};
