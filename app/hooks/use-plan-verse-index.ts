 ...
 * React Query key for users who also open the reader. Always resolves against
 * `CANONICAL_PAGE_MUSHAF_ID` (mushaf 2) — plan ranges are always expressed
 * against that edition's page layout (ADR 0038), independent of both the
 * reader's active edition and `DEFAULT_MUSHAF_ID` (ADR 0066).
 *
...
const buildIndex = async (): Promise<PlanVerseIndex> => {
  const [chapters, versePages] = await Promise.all([
    fetchChapters(),
    fetchVersePages(CANONICAL_PAGE_MUSHAF_ID),
  ]);
