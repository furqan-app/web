# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: visual.spec.ts >> settings sheet (ar-light) >> open settings sheet
- Location: e2e/tests/visual.spec.ts:131:11

# Error details

```
Error: expect(page).toHaveScreenshot(expected) failed

  61201 pixels (ratio 0.06 of all image pixels) are different.

  Snapshot: settings-ar-light.png

Call log:
  - Expect "toHaveScreenshot(settings-ar-light.png)" with timeout 5000ms
    - verifying given screenshot expectation
  - taking page screenshot
    - disabled all CSS animations
  - waiting for fonts to load...
  - fonts loaded
  - 61201 pixels (ratio 0.06 of all image pixels) are different.
  - waiting 100ms before taking screenshot
  - taking page screenshot
    - disabled all CSS animations
  - waiting for fonts to load...
  - fonts loaded
  - captured a stable screenshot
  - 61201 pixels (ratio 0.06 of all image pixels) are different.

```

# Page snapshot

```yaml
- generic:
  - generic:
    - navigation:
      - generic:
        - link:
          - /url: /
        - link:
          - /url: /ar/pages/1
          - img
          - generic: متابعة القراءة
        - generic:
          - button:
            - img
          - button:
            - img
        - button:
          - img
        - generic:
          - button:
            - img
    - main:
      - generic:
        - generic:
          - generic: بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ
        - generic:
          - heading [level=1]: الفرقان
        - paragraph: كلُّ كلمة أمانة، وكلُّ آية رحلة — علِّق وتأمَّل وأتقن حفظك للقرآن الكريم، خطوةً بخطوة بجانب شيخك.
      - generic:
        - generic:
          - generic:
            - img
          - generic:
            - generic: تابع القراءة
            - generic:
              - generic: "001"
              - generic: • صفحة ١
        - link:
          - /url: /ar/pages/1
          - generic: متابعة التلاوة
          - img
      - region:
        - generic:
          - generic: السور الموصى بها
        - generic:
          - link:
            - /url: /ar/pages/1
            - generic: "001"
          - link:
            - /url: /ar/pages/293
            - generic: "018"
          - link:
            - /url: /ar/pages/440
            - generic: "036"
          - link:
            - /url: /ar/pages/531
            - generic: "055"
          - link:
            - /url: /ar/pages/534
            - generic: "056"
          - link:
            - /url: /ar/pages/562
            - generic: "067"
      - generic:
        - link:
          - /url: /ar/pages/1
          - generic:
            - generic:
              - img
              - generic: ١
            - generic:
              - generic: "001"
            - generic: مكية
          - generic:
            - generic: ٧ آيات
            - generic: صفحة ١
        - link:
          - /url: /ar/pages/2
          - generic:
            - generic:
              - img
              - generic: ٢
            - generic:
              - generic: "002"
            - generic: مدنية
          - generic:
            - generic: ٢٨٦ آية
            - generic: صفحة ٢
        - link:
          - /url: /ar/pages/50
          - generic:
            - generic:
              - img
              - generic: ٣
            - generic:
              - generic: "003"
            - generic: مدنية
          - generic:
            - generic: ٢٠٠ آية
            - generic: صفحة ٥٠
        - link:
          - /url: /ar/pages/77
          - generic:
            - generic:
              - img
              - generic: ٤
            - generic:
              - generic: "004"
            - generic: مدنية
          - generic:
            - generic: ١٧٦ آية
            - generic: صفحة ٧٧
        - link:
          - /url: /ar/pages/106
          - generic:
            - generic:
              - img
              - generic: ٥
            - generic:
              - generic: "005"
            - generic: مدنية
          - generic:
            - generic: ١٢٠ آية
            - generic: صفحة ١٠٦
        - link:
          - /url: /ar/pages/128
          - generic:
            - generic:
              - img
              - generic: ٦
            - generic:
              - generic: "006"
            - generic: مكية
          - generic:
            - generic: ١٦٥ آية
            - generic: صفحة ١٢٨
        - link:
          - /url: /ar/pages/151
          - generic:
            - generic:
              - img
              - generic: ٧
            - generic:
              - generic: "007"
            - generic: مكية
          - generic:
            - generic: ٢٠٦ آية
            - generic: صفحة ١٥١
        - link:
          - /url: /ar/pages/177
          - generic:
            - generic:
              - img
              - generic: ٨
            - generic:
              - generic: "008"
            - generic: مدنية
          - generic:
            - generic: ٧٥ آية
            - generic: صفحة ١٧٧
        - link:
          - /url: /ar/pages/187
          - generic:
            - generic:
              - img
              - generic: ٩
            - generic:
              - generic: "009"
            - generic: مدنية
          - generic:
            - generic: ١٢٩ آية
            - generic: صفحة ١٨٧
        - link:
          - /url: /ar/pages/208
          - generic:
            - generic:
              - img
              - generic: ١٠
            - generic:
              - generic: "010"
            - generic: مكية
          - generic:
            - generic: ١٠٩ آية
            - generic: صفحة ٢٠٨
        - link:
          - /url: /ar/pages/221
          - generic:
            - generic:
              - img
              - generic: ١١
            - generic:
              - generic: "011"
            - generic: مكية
          - generic:
            - generic: ١٢٣ آية
            - generic: صفحة ٢٢١
        - link:
          - /url: /ar/pages/235
          - generic:
            - generic:
              - img
              - generic: ١٢
            - generic:
              - generic: "012"
            - generic: مكية
          - generic:
            - generic: ١١١ آية
            - generic: صفحة ٢٣٥
        - link:
          - /url: /ar/pages/249
          - generic:
            - generic:
              - img
              - generic: ١٣
            - generic:
              - generic: "013"
            - generic: مدنية
          - generic:
            - generic: ٤٣ آية
            - generic: صفحة ٢٤٩
        - link:
          - /url: /ar/pages/255
          - generic:
            - generic:
              - img
              - generic: ١٤
            - generic:
              - generic: "014"
            - generic: مكية
          - generic:
            - generic: ٥٢ آية
            - generic: صفحة ٢٥٥
        - link:
          - /url: /ar/pages/262
          - generic:
            - generic:
              - img
              - generic: ١٥
            - generic:
              - generic: "015"
            - generic: مكية
          - generic:
            - generic: ٩٩ آية
            - generic: صفحة ٢٦٢
        - link:
          - /url: /ar/pages/267
          - generic:
            - generic:
              - img
              - generic: ١٦
            - generic:
              - generic: "016"
            - generic: مكية
          - generic:
            - generic: ١٢٨ آية
            - generic: صفحة ٢٦٧
        - link:
          - /url: /ar/pages/282
          - generic:
            - generic:
              - img
              - generic: ١٧
            - generic:
              - generic: "017"
            - generic: مكية
          - generic:
            - generic: ١١١ آية
            - generic: صفحة ٢٨٢
        - link:
          - /url: /ar/pages/293
          - generic:
            - generic:
              - img
              - generic: ١٨
            - generic:
              - generic: "018"
            - generic: مكية
          - generic:
            - generic: ١١٠ آية
            - generic: صفحة ٢٩٣
        - link:
          - /url: /ar/pages/305
          - generic:
            - generic:
              - img
              - generic: ١٩
            - generic:
              - generic: "019"
            - generic: مكية
          - generic:
            - generic: ٩٨ آية
            - generic: صفحة ٣٠٥
        - link:
          - /url: /ar/pages/312
          - generic:
            - generic:
              - img
              - generic: ٢٠
            - generic:
              - generic: "020"
            - generic: مكية
          - generic:
            - generic: ١٣٥ آية
            - generic: صفحة ٣١٢
        - link:
          - /url: /ar/pages/322
          - generic:
            - generic:
              - img
              - generic: ٢١
            - generic:
              - generic: "021"
            - generic: مكية
          - generic:
            - generic: ١١٢ آية
            - generic: صفحة ٣٢٢
        - link:
          - /url: /ar/pages/332
          - generic:
            - generic:
              - img
              - generic: ٢٢
            - generic:
              - generic: "022"
            - generic: مدنية
          - generic:
            - generic: ٧٨ آية
            - generic: صفحة ٣٣٢
        - link:
          - /url: /ar/pages/342
          - generic:
            - generic:
              - img
              - generic: ٢٣
            - generic:
              - generic: "023"
            - generic: مكية
          - generic:
            - generic: ١١٨ آية
            - generic: صفحة ٣٤٢
        - link:
          - /url: /ar/pages/350
          - generic:
            - generic:
              - img
              - generic: ٢٤
            - generic:
              - generic: "024"
            - generic: مدنية
          - generic:
            - generic: ٦٤ آية
            - generic: صفحة ٣٥٠
        - link:
          - /url: /ar/pages/359
          - generic:
            - generic:
              - img
              - generic: ٢٥
            - generic:
              - generic: "025"
            - generic: مكية
          - generic:
            - generic: ٧٧ آية
            - generic: صفحة ٣٥٩
        - link:
          - /url: /ar/pages/367
          - generic:
            - generic:
              - img
              - generic: ٢٦
            - generic:
              - generic: "026"
            - generic: مكية
          - generic:
            - generic: ٢٢٧ آية
            - generic: صفحة ٣٦٧
        - link:
          - /url: /ar/pages/377
          - generic:
            - generic:
              - img
              - generic: ٢٧
            - generic:
              - generic: "027"
            - generic: مكية
          - generic:
            - generic: ٩٣ آية
            - generic: صفحة ٣٧٧
        - link:
          - /url: /ar/pages/385
          - generic:
            - generic:
              - img
              - generic: ٢٨
            - generic:
              - generic: "028"
            - generic: مكية
          - generic:
            - generic: ٨٨ آية
            - generic: صفحة ٣٨٥
        - link:
          - /url: /ar/pages/396
          - generic:
            - generic:
              - img
              - generic: ٢٩
            - generic:
              - generic: "029"
            - generic: مكية
          - generic:
            - generic: ٦٩ آية
            - generic: صفحة ٣٩٦
        - link:
          - /url: /ar/pages/404
          - generic:
            - generic:
              - img
              - generic: ٣٠
            - generic:
              - generic: "030"
            - generic: مكية
          - generic:
            - generic: ٦٠ آية
            - generic: صفحة ٤٠٤
        - link:
          - /url: /ar/pages/411
          - generic:
            - generic:
              - img
              - generic: ٣١
            - generic:
              - generic: "031"
            - generic: مكية
          - generic:
            - generic: ٣٤ آية
            - generic: صفحة ٤١١
        - link:
          - /url: /ar/pages/415
          - generic:
            - generic:
              - img
              - generic: ٣٢
            - generic:
              - generic: "032"
            - generic: مكية
          - generic:
            - generic: ٣٠ آية
            - generic: صفحة ٤١٥
        - link:
          - /url: /ar/pages/418
          - generic:
            - generic:
              - img
              - generic: ٣٣
            - generic:
              - generic: "033"
            - generic: مدنية
          - generic:
            - generic: ٧٣ آية
            - generic: صفحة ٤١٨
        - link:
          - /url: /ar/pages/428
          - generic:
            - generic:
              - img
              - generic: ٣٤
            - generic:
              - generic: "034"
            - generic: مكية
          - generic:
            - generic: ٥٤ آية
            - generic: صفحة ٤٢٨
        - link:
          - /url: /ar/pages/434
          - generic:
            - generic:
              - img
              - generic: ٣٥
            - generic:
              - generic: "035"
            - generic: مكية
          - generic:
            - generic: ٤٥ آية
            - generic: صفحة ٤٣٤
        - link:
          - /url: /ar/pages/440
          - generic:
            - generic:
              - img
              - generic: ٣٦
            - generic:
              - generic: "036"
            - generic: مكية
          - generic:
            - generic: ٨٣ آية
            - generic: صفحة ٤٤٠
        - link:
          - /url: /ar/pages/446
          - generic:
            - generic:
              - img
              - generic: ٣٧
            - generic:
              - generic: "037"
            - generic: مكية
          - generic:
            - generic: ١٨٢ آية
            - generic: صفحة ٤٤٦
        - link:
          - /url: /ar/pages/453
          - generic:
            - generic:
              - img
              - generic: ٣٨
            - generic:
              - generic: "038"
            - generic: مكية
          - generic:
            - generic: ٨٨ آية
            - generic: صفحة ٤٥٣
        - link:
          - /url: /ar/pages/458
          - generic:
            - generic:
              - img
              - generic: ٣٩
            - generic:
              - generic: "039"
            - generic: مكية
          - generic:
            - generic: ٧٥ آية
            - generic: صفحة ٤٥٨
        - link:
          - /url: /ar/pages/467
          - generic:
            - generic:
              - img
              - generic: ٤٠
            - generic:
              - generic: "040"
            - generic: مكية
          - generic:
            - generic: ٨٥ آية
            - generic: صفحة ٤٦٧
        - link:
          - /url: /ar/pages/477
          - generic:
            - generic:
              - img
              - generic: ٤١
            - generic:
              - generic: "041"
            - generic: مكية
          - generic:
            - generic: ٥٤ آية
            - generic: صفحة ٤٧٧
        - link:
          - /url: /ar/pages/483
          - generic:
            - generic:
              - img
              - generic: ٤٢
            - generic:
              - generic: "042"
            - generic: مكية
          - generic:
            - generic: ٥٣ آية
            - generic: صفحة ٤٨٣
        - link:
          - /url: /ar/pages/489
          - generic:
            - generic:
              - img
              - generic: ٤٣
            - generic:
              - generic: "043"
            - generic: مكية
          - generic:
            - generic: ٨٩ آية
            - generic: صفحة ٤٨٩
        - link:
          - /url: /ar/pages/496
          - generic:
            - generic:
              - img
              - generic: ٤٤
            - generic:
              - generic: "044"
            - generic: مكية
          - generic:
            - generic: ٥٩ آية
            - generic: صفحة ٤٩٦
        - link:
          - /url: /ar/pages/499
          - generic:
            - generic:
              - img
              - generic: ٤٥
            - generic:
              - generic: "045"
            - generic: مكية
          - generic:
            - generic: ٣٧ آية
            - generic: صفحة ٤٩٩
        - link:
          - /url: /ar/pages/502
          - generic:
            - generic:
              - img
              - generic: ٤٦
            - generic:
              - generic: "046"
            - generic: مكية
          - generic:
            - generic: ٣٥ آية
            - generic: صفحة ٥٠٢
        - link:
          - /url: /ar/pages/507
          - generic:
            - generic:
              - img
              - generic: ٤٧
            - generic:
              - generic: "047"
            - generic: مدنية
          - generic:
            - generic: ٣٨ آية
            - generic: صفحة ٥٠٧
        - link:
          - /url: /ar/pages/511
          - generic:
            - generic:
              - img
              - generic: ٤٨
            - generic:
              - generic: "048"
            - generic: مدنية
          - generic:
            - generic: ٢٩ آية
            - generic: صفحة ٥١١
        - link:
          - /url: /ar/pages/515
          - generic:
            - generic:
              - img
              - generic: ٤٩
            - generic:
              - generic: "049"
            - generic: مدنية
          - generic:
            - generic: ١٨ آية
            - generic: صفحة ٥١٥
        - link:
          - /url: /ar/pages/518
          - generic:
            - generic:
              - img
              - generic: ٥٠
            - generic:
              - generic: "050"
            - generic: مكية
          - generic:
            - generic: ٤٥ آية
            - generic: صفحة ٥١٨
        - link:
          - /url: /ar/pages/520
          - generic:
            - generic:
              - img
              - generic: ٥١
            - generic:
              - generic: "051"
            - generic: مكية
          - generic:
            - generic: ٦٠ آية
            - generic: صفحة ٥٢٠
        - link:
          - /url: /ar/pages/523
          - generic:
            - generic:
              - img
              - generic: ٥٢
            - generic:
              - generic: "052"
            - generic: مكية
          - generic:
            - generic: ٤٩ آية
            - generic: صفحة ٥٢٣
        - link:
          - /url: /ar/pages/526
          - generic:
            - generic:
              - img
              - generic: ٥٣
            - generic:
              - generic: "053"
            - generic: مكية
          - generic:
            - generic: ٦٢ آية
            - generic: صفحة ٥٢٦
        - link:
          - /url: /ar/pages/528
          - generic:
            - generic:
              - img
              - generic: ٥٤
            - generic:
              - generic: "054"
            - generic: مكية
          - generic:
            - generic: ٥٥ آية
            - generic: صفحة ٥٢٨
        - link:
          - /url: /ar/pages/531
          - generic:
            - generic:
              - img
              - generic: ٥٥
            - generic:
              - generic: "055"
            - generic: مدنية
          - generic:
            - generic: ٧٨ آية
            - generic: صفحة ٥٣١
        - link:
          - /url: /ar/pages/534
          - generic:
            - generic:
              - img
              - generic: ٥٦
            - generic:
              - generic: "056"
            - generic: مكية
          - generic:
            - generic: ٩٦ آية
            - generic: صفحة ٥٣٤
        - link:
          - /url: /ar/pages/537
          - generic:
            - generic:
              - img
              - generic: ٥٧
            - generic:
              - generic: "057"
            - generic: مدنية
          - generic:
            - generic: ٢٩ آية
            - generic: صفحة ٥٣٧
        - link:
          - /url: /ar/pages/542
          - generic:
            - generic:
              - img
              - generic: ٥٨
            - generic:
              - generic: "058"
            - generic: مدنية
          - generic:
            - generic: ٢٢ آية
            - generic: صفحة ٥٤٢
        - link:
          - /url: /ar/pages/545
          - generic:
            - generic:
              - img
              - generic: ٥٩
            - generic:
              - generic: "059"
            - generic: مدنية
          - generic:
            - generic: ٢٤ آية
            - generic: صفحة ٥٤٥
        - link:
          - /url: /ar/pages/549
          - generic:
            - generic:
              - img
              - generic: ٦٠
            - generic:
              - generic: "060"
            - generic: مدنية
          - generic:
            - generic: ١٣ آية
            - generic: صفحة ٥٤٩
        - link:
          - /url: /ar/pages/551
          - generic:
            - generic:
              - img
              - generic: ٦١
            - generic:
              - generic: "061"
            - generic: مدنية
          - generic:
            - generic: ١٤ آية
            - generic: صفحة ٥٥١
        - link:
          - /url: /ar/pages/553
          - generic:
            - generic:
              - img
              - generic: ٦٢
            - generic:
              - generic: "062"
            - generic: مدنية
          - generic:
            - generic: ١١ آية
            - generic: صفحة ٥٥٣
        - link:
          - /url: /ar/pages/554
          - generic:
            - generic:
              - img
              - generic: ٦٣
            - generic:
              - generic: "063"
            - generic: مدنية
          - generic:
            - generic: ١١ آية
            - generic: صفحة ٥٥٤
        - link:
          - /url: /ar/pages/556
          - generic:
            - generic:
              - img
              - generic: ٦٤
            - generic:
              - generic: "064"
            - generic: مدنية
          - generic:
            - generic: ١٨ آية
            - generic: صفحة ٥٥٦
        - link:
          - /url: /ar/pages/558
          - generic:
            - generic:
              - img
              - generic: ٦٥
            - generic:
              - generic: "065"
            - generic: مدنية
          - generic:
            - generic: ١٢ آية
            - generic: صفحة ٥٥٨
        - link:
          - /url: /ar/pages/560
          - generic:
            - generic:
              - img
              - generic: ٦٦
            - generic:
              - generic: "066"
            - generic: مدنية
          - generic:
            - generic: ١٢ آية
            - generic: صفحة ٥٦٠
        - link:
          - /url: /ar/pages/562
          - generic:
            - generic:
              - img
              - generic: ٦٧
            - generic:
              - generic: "067"
            - generic: مكية
          - generic:
            - generic: ٣٠ آية
            - generic: صفحة ٥٦٢
        - link:
          - /url: /ar/pages/564
          - generic:
            - generic:
              - img
              - generic: ٦٨
            - generic:
              - generic: "068"
            - generic: مكية
          - generic:
            - generic: ٥٢ آية
            - generic: صفحة ٥٦٤
        - link:
          - /url: /ar/pages/566
          - generic:
            - generic:
              - img
              - generic: ٦٩
            - generic:
              - generic: "069"
            - generic: مكية
          - generic:
            - generic: ٥٢ آية
            - generic: صفحة ٥٦٦
        - link:
          - /url: /ar/pages/568
          - generic:
            - generic:
              - img
              - generic: ٧٠
            - generic:
              - generic: "070"
            - generic: مكية
          - generic:
            - generic: ٤٤ آية
            - generic: صفحة ٥٦٨
        - link:
          - /url: /ar/pages/570
          - generic:
            - generic:
              - img
              - generic: ٧١
            - generic:
              - generic: "071"
            - generic: مكية
          - generic:
            - generic: ٢٨ آية
            - generic: صفحة ٥٧٠
        - link:
          - /url: /ar/pages/572
          - generic:
            - generic:
              - img
              - generic: ٧٢
            - generic:
              - generic: "072"
            - generic: مكية
          - generic:
            - generic: ٢٨ آية
            - generic: صفحة ٥٧٢
        - link:
          - /url: /ar/pages/574
          - generic:
            - generic:
              - img
              - generic: ٧٣
            - generic:
              - generic: "073"
            - generic: مكية
          - generic:
            - generic: ٢٠ آية
            - generic: صفحة ٥٧٤
        - link:
          - /url: /ar/pages/575
          - generic:
            - generic:
              - img
              - generic: ٧٤
            - generic:
              - generic: "074"
            - generic: مكية
          - generic:
            - generic: ٥٦ آية
            - generic: صفحة ٥٧٥
        - link:
          - /url: /ar/pages/577
          - generic:
            - generic:
              - img
              - generic: ٧٥
            - generic:
              - generic: "075"
            - generic: مكية
          - generic:
            - generic: ٤٠ آية
            - generic: صفحة ٥٧٧
        - link:
          - /url: /ar/pages/578
          - generic:
            - generic:
              - img
              - generic: ٧٦
            - generic:
              - generic: "076"
            - generic: مدنية
          - generic:
            - generic: ٣١ آية
            - generic: صفحة ٥٧٨
        - link:
          - /url: /ar/pages/580
          - generic:
            - generic:
              - img
              - generic: ٧٧
            - generic:
              - generic: "077"
            - generic: مكية
          - generic:
            - generic: ٥٠ آية
            - generic: صفحة ٥٨٠
        - link:
          - /url: /ar/pages/582
          - generic:
            - generic:
              - img
              - generic: ٧٨
            - generic:
              - generic: "078"
            - generic: مكية
          - generic:
            - generic: ٤٠ آية
            - generic: صفحة ٥٨٢
        - link:
          - /url: /ar/pages/583
          - generic:
            - generic:
              - img
              - generic: ٧٩
            - generic:
              - generic: "079"
            - generic: مكية
          - generic:
            - generic: ٤٦ آية
            - generic: صفحة ٥٨٣
        - link:
          - /url: /ar/pages/585
          - generic:
            - generic:
              - img
              - generic: ٨٠
            - generic:
              - generic: "080"
            - generic: مكية
          - generic:
            - generic: ٤٢ آية
            - generic: صفحة ٥٨٥
        - link:
          - /url: /ar/pages/586
          - generic:
            - generic:
              - img
              - generic: ٨١
            - generic:
              - generic: "081"
            - generic: مكية
          - generic:
            - generic: ٢٩ آية
            - generic: صفحة ٥٨٦
        - link:
          - /url: /ar/pages/587
          - generic:
            - generic:
              - img
              - generic: ٨٢
            - generic:
              - generic: "082"
            - generic: مكية
          - generic:
            - generic: ١٩ آية
            - generic: صفحة ٥٨٧
        - link:
          - /url: /ar/pages/587
          - generic:
            - generic:
              - img
              - generic: ٨٣
            - generic:
              - generic: "083"
            - generic: مكية
          - generic:
            - generic: ٣٦ آية
            - generic: صفحة ٥٨٧
        - link:
          - /url: /ar/pages/589
          - generic:
            - generic:
              - img
              - generic: ٨٤
            - generic:
              - generic: "084"
            - generic: مكية
          - generic:
            - generic: ٢٥ آية
            - generic: صفحة ٥٨٩
        - link:
          - /url: /ar/pages/590
          - generic:
            - generic:
              - img
              - generic: ٨٥
            - generic:
              - generic: "085"
            - generic: مكية
          - generic:
            - generic: ٢٢ آية
            - generic: صفحة ٥٩٠
        - link:
          - /url: /ar/pages/591
          - generic:
            - generic:
              - img
              - generic: ٨٦
            - generic:
              - generic: "086"
            - generic: مكية
          - generic:
            - generic: ١٧ آية
            - generic: صفحة ٥٩١
        - link:
          - /url: /ar/pages/591
          - generic:
            - generic:
              - img
              - generic: ٨٧
            - generic:
              - generic: "087"
            - generic: مكية
          - generic:
            - generic: ١٩ آية
            - generic: صفحة ٥٩١
        - link:
          - /url: /ar/pages/592
          - generic:
            - generic:
              - img
              - generic: ٨٨
            - generic:
              - generic: "088"
            - generic: مكية
          - generic:
            - generic: ٢٦ آية
            - generic: صفحة ٥٩٢
        - link:
          - /url: /ar/pages/593
          - generic:
            - generic:
              - img
              - generic: ٨٩
            - generic:
              - generic: "089"
            - generic: مكية
          - generic:
            - generic: ٣٠ آية
            - generic: صفحة ٥٩٣
        - link:
          - /url: /ar/pages/594
          - generic:
            - generic:
              - img
              - generic: ٩٠
            - generic:
              - generic: "090"
            - generic: مكية
          - generic:
            - generic: ٢٠ آية
            - generic: صفحة ٥٩٤
        - link:
          - /url: /ar/pages/595
          - generic:
            - generic:
              - img
              - generic: ٩١
            - generic:
              - generic: "091"
            - generic: مكية
          - generic:
            - generic: ١٥ آية
            - generic: صفحة ٥٩٥
        - link:
          - /url: /ar/pages/595
          - generic:
            - generic:
              - img
              - generic: ٩٢
            - generic:
              - generic: "092"
            - generic: مكية
          - generic:
            - generic: ٢١ آية
            - generic: صفحة ٥٩٥
        - link:
          - /url: /ar/pages/596
          - generic:
            - generic:
              - img
              - generic: ٩٣
            - generic:
              - generic: "093"
            - generic: مكية
          - generic:
            - generic: ١١ آية
            - generic: صفحة ٥٩٦
        - link:
          - /url: /ar/pages/596
          - generic:
            - generic:
              - img
              - generic: ٩٤
            - generic:
              - generic: "094"
            - generic: مكية
          - generic:
            - generic: ٨ آيات
            - generic: صفحة ٥٩٦
        - link:
          - /url: /ar/pages/597
          - generic:
            - generic:
              - img
              - generic: ٩٥
            - generic:
              - generic: "095"
            - generic: مكية
          - generic:
            - generic: ٨ آيات
            - generic: صفحة ٥٩٧
        - link:
          - /url: /ar/pages/597
          - generic:
            - generic:
              - img
              - generic: ٩٦
            - generic:
              - generic: "096"
            - generic: مكية
          - generic:
            - generic: ١٩ آية
            - generic: صفحة ٥٩٧
        - link:
          - /url: /ar/pages/598
          - generic:
            - generic:
              - img
              - generic: ٩٧
            - generic:
              - generic: "097"
            - generic: مكية
          - generic:
            - generic: ٥ آيات
            - generic: صفحة ٥٩٨
        - link:
          - /url: /ar/pages/598
          - generic:
            - generic:
              - img
              - generic: ٩٨
            - generic:
              - generic: "098"
            - generic: مدنية
          - generic:
            - generic: ٨ آيات
            - generic: صفحة ٥٩٨
        - link:
          - /url: /ar/pages/599
          - generic:
            - generic:
              - img
              - generic: ٩٩
            - generic:
              - generic: "099"
            - generic: مدنية
          - generic:
            - generic: ٨ آيات
            - generic: صفحة ٥٩٩
        - link:
          - /url: /ar/pages/599
          - generic:
            - generic:
              - img
              - generic: ١٠٠
            - generic:
              - generic: "100"
            - generic: مكية
          - generic:
            - generic: ١١ آية
            - generic: صفحة ٥٩٩
        - link:
          - /url: /ar/pages/600
          - generic:
            - generic:
              - img
              - generic: ١٠١
            - generic:
              - generic: "101"
            - generic: مكية
          - generic:
            - generic: ١١ آية
            - generic: صفحة ٦٠٠
        - link:
          - /url: /ar/pages/600
          - generic:
            - generic:
              - img
              - generic: ١٠٢
            - generic:
              - generic: "102"
            - generic: مكية
          - generic:
            - generic: ٨ آيات
            - generic: صفحة ٦٠٠
        - link:
          - /url: /ar/pages/601
          - generic:
            - generic:
              - img
              - generic: ١٠٣
            - generic:
              - generic: "103"
            - generic: مكية
          - generic:
            - generic: ٣ آيات
            - generic: صفحة ٦٠١
        - link:
          - /url: /ar/pages/601
          - generic:
            - generic:
              - img
              - generic: ١٠٤
            - generic:
              - generic: "104"
            - generic: مكية
          - generic:
            - generic: ٩ آيات
            - generic: صفحة ٦٠١
        - link:
          - /url: /ar/pages/601
          - generic:
            - generic:
              - img
              - generic: ١٠٥
            - generic:
              - generic: "105"
            - generic: مكية
          - generic:
            - generic: ٥ آيات
            - generic: صفحة ٦٠١
        - link:
          - /url: /ar/pages/602
          - generic:
            - generic:
              - img
              - generic: ١٠٦
            - generic:
              - generic: "106"
            - generic: مكية
          - generic:
            - generic: ٤ آيات
            - generic: صفحة ٦٠٢
        - link:
          - /url: /ar/pages/602
          - generic:
            - generic:
              - img
              - generic: ١٠٧
            - generic:
              - generic: "107"
            - generic: مكية
          - generic:
            - generic: ٧ آيات
            - generic: صفحة ٦٠٢
        - link:
          - /url: /ar/pages/602
          - generic:
            - generic:
              - img
              - generic: ١٠٨
            - generic:
              - generic: "108"
            - generic: مكية
          - generic:
            - generic: ٣ آيات
            - generic: صفحة ٦٠٢
        - link:
          - /url: /ar/pages/603
          - generic:
            - generic:
              - img
              - generic: ١٠٩
            - generic:
              - generic: "109"
            - generic: مكية
          - generic:
            - generic: ٦ آيات
            - generic: صفحة ٦٠٣
        - link:
          - /url: /ar/pages/603
          - generic:
            - generic:
              - img
              - generic: ١١٠
            - generic:
              - generic: "110"
            - generic: مدنية
          - generic:
            - generic: ٣ آيات
            - generic: صفحة ٦٠٣
        - link:
          - /url: /ar/pages/603
          - generic:
            - generic:
              - img
              - generic: ١١١
            - generic:
              - generic: "111"
            - generic: مكية
          - generic:
            - generic: ٥ آيات
            - generic: صفحة ٦٠٣
        - link:
          - /url: /ar/pages/604
          - generic:
            - generic:
              - img
              - generic: ١١٢
            - generic:
              - generic: "112"
            - generic: مكية
          - generic:
            - generic: ٤ آيات
            - generic: صفحة ٦٠٤
        - link:
          - /url: /ar/pages/604
          - generic:
            - generic:
              - img
              - generic: ١١٣
            - generic:
              - generic: "113"
            - generic: مكية
          - generic:
            - generic: ٥ آيات
            - generic: صفحة ٦٠٤
        - link:
          - /url: /ar/pages/604
          - generic:
            - generic:
              - img
              - generic: ١١٤
            - generic:
              - generic: "114"
            - generic: مكية
          - generic:
            - generic: ٦ آيات
            - generic: صفحة ٦٠٤
  - alert
  - dialog "الإعدادات" [ref=e2]:
    - generic [ref=e3]:
      - heading "الإعدادات" [level=2] [ref=e4]
      - paragraph [ref=e5]: عدّل اللغة وحجم الخط والمظهر والوصول دون اتصال.
    - generic [ref=e6]:
      - generic [ref=e7]:
        - generic [ref=e8]: القراءة
        - generic [ref=e9]:
          - button "اللغة العربية" [active] [ref=e11] [cursor=pointer]:
            - generic [ref=e12]:
              - text: اللغة
              - paragraph [ref=e13]: العربية
            - img [ref=e14]
          - button "تخطيط المصحف مصحف مجمع الملك فهد (طبعة ١٤٠٥هـ)" [ref=e17] [cursor=pointer]:
            - generic [ref=e18]:
              - text: تخطيط المصحف
              - paragraph [ref=e19]: مصحف مجمع الملك فهد (طبعة ١٤٠٥هـ)
            - img [ref=e20]
      - generic [ref=e22]:
        - generic [ref=e23]: المظهر
        - radiogroup "theme.appearance" [ref=e25]:
          - radio "فاتح" [checked] [ref=e26] [cursor=pointer]:
            - generic [ref=e28]: فاتح
            - img [ref=e30]
          - radio "ذهبي" [ref=e32] [cursor=pointer]:
            - generic [ref=e34]: ذهبي
            - img [ref=e36]
          - radio "داكن" [ref=e38] [cursor=pointer]:
            - generic [ref=e40]: داكن
            - img [ref=e42]
      - generic [ref=e44]:
        - generic [ref=e45]: الجهاز والتلاوة
        - generic [ref=e46]:
          - generic [ref=e47]:
            - generic [ref=e48] [cursor=pointer]:
              - text: إبقاء الشاشة مضاءة
              - paragraph [ref=e49]: منع شاشة جهازك من الإغلاق التلقائي أثناء فتح التطبيق
            - switch "إبقاء الشاشة مضاءة منع شاشة جهازك من الإغلاق التلقائي أثناء فتح التطبيق" [checked] [ref=e50] [cursor=pointer]
          - generic [ref=e51]:
            - generic [ref=e52]:
              - text: إشعارات الدفع
              - paragraph [ref=e53]: محظورة من المتصفح — اسمح بالإشعارات في إعدادات الموقع لتفعيلها.
            - switch "إشعارات الدفع محظورة من المتصفح — اسمح بالإشعارات في إعدادات الموقع لتفعيلها." [disabled] [ref=e55]
    - button "Close" [ref=e58] [cursor=pointer]:
      - img [ref=e59]
      - generic [ref=e62]: Close
```

# Test source

```ts
  43  | /** Sets the theme in localStorage before first paint, mirroring app/utils/storage.ts's JSON.stringify shape. */
  44  | async function withTheme(page: Page, theme: Theme) {
  45  |   await page.addInitScript((t) => {
  46  |     window.localStorage.setItem("theme", JSON.stringify(t));
  47  |   }, theme);
  48  | }
  49  | 
  50  | // Blocks until every mounted safha has painted its word rows.
  51  | //
  52  | // `toHaveScreenshot` decides a page has settled by comparing two screenshots
  53  | // 100ms apart — but it disables CSS animations first, which freezes the loading
  54  | // skeleton's `animate-pulse` and makes a half-loaded reader look like a settled
  55  | // one. Its only built-in readiness signal, `document.fonts.ready`, resolves long
  56  | // before the line content arrives (ADR 0034), so without this the captured frame
  57  | // is whichever of {skeleton, text} the runner happened to reach — measured at
  58  | // 4-in-8 runs, and the difference between the two lands right on the diff gate.
  59  | //
  60  | // This asserts content is PRESENT rather than that the skeleton is absent. A
  61  | // `no .animate-pulse` check returns an empty list — and so passes instantly —
  62  | // the moment that class is renamed, silently restoring the flake with no failing
  63  | // test to reveal it. The length guard is the same hazard one level up: `every()`
  64  | // on an empty list is vacuously true. See docs/plans/visual-e2e-testing.md
  65  | // Addendum (2026-08-02).
  66  | async function waitForReaderContent(page: Page) {
  67  |   await page.waitForFunction(() => {
  68  |     const safhas = Array.from(document.querySelectorAll(".fq-quran-safha"));
  69  |     return safhas.length > 0 && safhas.every((el) => el.querySelector(".fq-safha-row"));
  70  |   });
  71  | }
  72  | 
  73  | for (const locale of LOCALES) {
  74  |   for (const theme of THEMES) {
  75  |     const suffix = `${locale}-${theme}`;
  76  | 
  77  |     test.describe(`home (${suffix})`, () => {
  78  |       test("surah list", async ({ page }) => {
  79  |         await withTheme(page, theme);
  80  |         await page.goto(`/${locale}`);
  81  |         await expect(page).toHaveScreenshot(`home-${suffix}.png`);
  82  |       });
  83  |     });
  84  | 
  85  |     test.describe(`quran page 1 (${suffix})`, () => {
  86  |       test("single page, short opening page", async ({ page }) => {
  87  |         await withTheme(page, theme);
  88  |         await page.goto(`/${locale}/pages/1`);
  89  |         await waitForReaderContent(page);
  90  |         await expect(page).toHaveScreenshot(`page-1-${suffix}.png`);
  91  |       });
  92  |     });
  93  | 
  94  |     test.describe(`quran pages 2-3 double-spread (${suffix})`, () => {
  95  |       test("double-page spread", async ({ page }, testInfo) => {
  96  |         test.skip(
  97  |           testInfo.project.name === "mobile",
  98  |           "double-page spread only renders at lg+ (ADR 0013) — nothing distinct to capture on mobile"
  99  |         );
  100 |         await withTheme(page, theme);
  101 |         await page.goto(`/${locale}/pages/2`);
  102 |         await waitForReaderContent(page);
  103 |         await expect(page).toHaveScreenshot(`spread-2-3-${suffix}.png`);
  104 |       });
  105 |     });
  106 | 
  107 |     test.describe(`search results (${suffix})`, () => {
  108 |       test("search for a chapter", async ({ page }) => {
  109 |         await withTheme(page, theme);
  110 |         await page.goto(`/${locale}`);
  111 | 
  112 |         // Search is a single icon trigger at every breakpoint (desktop no
  113 |         // longer has a persistent inline field — SearchBar.tsx,
  114 |         // docs/plans/desktop-navbar-font-bg.md) that opens the same
  115 |         // full-screen Sheet/dialog overlay on both desktop and mobile.
  116 |         await page.getByRole("button", { name: SEARCH_PLACEHOLDER[locale] }).click();
  117 |         const scope = page.getByRole("dialog");
  118 |         await scope.getByPlaceholder(SEARCH_PLACEHOLDER[locale]).fill(SEARCH_QUERY[locale]);
  119 | 
  120 |         // Positive wait on the rendered results rather than a fixed sleep: the
  121 |         // old `waitForTimeout(800)` left only ~300ms after the 500ms debounce for
  122 |         // the request and render, so the screenshot caught either the spinner or
  123 |         // the dropdown depending on timing — 2 of the 4 search snapshots differed
  124 |         // run-to-run at a ratio of ~0.05, well past the diff gate.
  125 |         await expect(scope.getByText(SEARCH_RESULTS_HEADING[locale])).toBeVisible();
  126 |         await expect(page).toHaveScreenshot(`search-${suffix}.png`);
  127 |       });
  128 |     });
  129 | 
  130 |     test.describe(`settings sheet (${suffix})`, () => {
  131 |       test("open settings sheet", async ({ page }, testInfo) => {
  132 |         await withTheme(page, theme);
  133 |         await page.goto(`/${locale}`);
  134 |         // Settings is in the nav row on desktop (as a button), but behind the UserMenu on mobile (as a menuitem).
  135 |         if (testInfo.project.name === "mobile") {
  136 |           await page.getByRole("button", { name: ACCOUNT_LABEL[locale] }).click();
  137 |           await page.getByRole("menuitem", { name: SETTINGS_LABEL[locale] }).click();
  138 |         } else {
  139 |           await page.getByRole("button", { name: SETTINGS_LABEL[locale] }).click();
  140 |         }
  141 |         // Sheet slide-in animation.
  142 |         await page.waitForTimeout(600);
> 143 |         await expect(page).toHaveScreenshot(`settings-${suffix}.png`);
      |                            ^ Error: expect(page).toHaveScreenshot(expected) failed
  144 |       });
  145 |     });
  146 |   }
  147 | }
  148 | 
```